...
export default class App extends Component<{}, IAppState> {
  ...
  setupTicks = () => {
    let speed: number = 750 - this.state.level * 50;
    if (speed < 350) {
      speed = 350;
    }
  ...
  popRandomMole = () => {
    if (this.moles.length === 12) {
      let randomIndex = this.randomBetween(0, this.moles.length - 1);
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.playArea}>
          {Array.apply(null, Array(4)).map((_, rowIdx: number) =>
              <View style={styles.playRow} key={rowIdx}>
                {Array.apply(null, Array(3)).map((_, colIdx: number) => {
